...
public class ManagerView extends UserView {
	...
	protected ManagerView(User user, FileController fileController, Creator creator,
			...
			ContractController contractControllerEmployee) {
	...
	public void start() {
		try {
			managerProduct = ((ContractControllerProduct)this.contractControllerProduct).getContracterOfContractee(getUser().getUserName());
			menu();
	...
	public void assignPartToEmployee() {
		...
		try {
			IContainer<User> employeesOfManager =  ((ContractControllerEmployee)contractControllerEmployee).getContracterOfContractee(this.getUser().getUserName());
			String emptyProducts = ViewHelper.findManagerEmptyProducts(managerProduct,  (ContractControllerProduct)contractControllerProduct);
			String emptyEmployees = ViewHelper.findManagerEmptyEmployees(employeesOfManager, (ContractControllerProduct)contractControllerProduct); 
			...
			else
			{
				...
				userController.assignProduct(employee, part, fileController,(ContractControllerProduct)contractControllerProduct);
